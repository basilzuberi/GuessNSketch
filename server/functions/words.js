class Words{
    ...
    getRandomWord (){
        if(this.wordsList.length){
            let rnd = Math.floor(Math.random() * this.wordsList.length);
            let newWord = this.wordsList[rnd]
            this.deletePickedWord(this.wordsList[rnd]);
            return newWord;
        }else{
    ...
    deletePickedWord(word){
       this.wordsList = this.wordsList.filter(elem => {
           return elem !== word;
       });
    }
}
